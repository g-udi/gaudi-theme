...
GAUDI_DOCKERCOMPOSE_PREFIX="${GAUDI_DOCKERCOMPOSE_PREFIX:="$GAUDI_PROMPT_DEFAULT_PREFIX"}"
...
GAUDI_DOCKERCOMPOSE_TEXT_COLOR="${GAUDI_DOCKERCOMPOSE_TEXT_COLOR:="${GAUDI_WHITE}"}"
GAUDI_DOCKERCOMPOSE_TEXT_COLOR_UP="${GAUDI_DOCKERCOMPOSE_TEXT_COLOR_UP:="${GAUDI_WHITE}"}"
GAUDI_DOCKERCOMPOSE_TEXT_COLOR_DOWN="${GAUDI_DOCKERCOMPOSE_TEXT_COLOR_DOWN:="${GAUDI_WHITE}"}"
GAUDI_DOCKERCOMPOSE_COLOR="${GAUDI_DOCKERCOMPOSE_COLOR:="${BACKGROUND_GAUDI_GREEN}"}"
GAUDI_DOCKERCOMPOSE_DOWN_COLOR="${GAUDI_DOCKERCOMPOSE_DOWN_COLOR:="${BACKGROUND_GAUDI_RED}"}"
GAUDI_DOCKERCOMPOSE_PARTIAL_COLOR="${GAUDI_DOCKERCOMPOSE_PARTIAL_COLOR:="${BACKGROUND_GAUDI_ORANGE}"}"
# ------------------------------------------------------------------------------
# Section
# ------------------------------------------------------------------------------
...
gaudi_dockercompose () {
  ...
  local dockercompose_details
  dockercompose_details=$(docker-compose ps 2>/dev/null | tail -n+3 | while read -r line
    do
      local CONTAINER_NAME CONTAINER_LETTER _CONTAINER_SYMBOL CONTAINER_SYMBOL
      ...
      _CONTAINER_SYMBOL="GAUDI_DOCKERCOMPOSE_SYMBOL_${CONTAINER_NAME}"
  ...
  local _dockercompose_details="${dockercompose_details%%::*}"
  local total_containers
  total_containers=$(docker-compose ps 2>/dev/null | tail -n +3 | wc -l)
  local up_containers="${dockercompose_details%%|*}"
  local dockercompose_status_up="${_dockercompose_details#*|}"
  local dockercompose_status_down="${dockercompose_details#*::}"
