...
GAUDI_DOTNET_COLOR="${GAUDI_DOTNET_COLOR="$GAUDI_MAGENTA"}"
...
gaudi_dotnet () {
  ...
  local _csproj_files=(*.csproj) _xproj_files=(*.xproj) _sln_files=(*.sln)
  [[ -f project.json || -f global.json ||
     -e "${_csproj_files[0]}" || -e "${_xproj_files[0]}" || -e "${_sln_files[0]}"
  ]] || return
  ...
  dotnet_version="$(dotnet --version 2>/dev/null)" || return
  [[ -n "$dotnet_version" ]] || return
